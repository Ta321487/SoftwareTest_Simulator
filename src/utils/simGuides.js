...
const simGuideContent = {
  ...
  calculator: {
    action: '按公式计算并填写结果，然后提交',
    steps: ['核对题目参数', '按公式计算', '填写结果并提交'],
  },
  packet: {
  ...
  oncall: {
    action: '在值班面板完成操作后点击「确认提交」',
    steps: ['阅读告警/投诉背景', '在面板里复现或标记已阅', '完成后点击底部提交'],
  },
  leadboard: {
    action: '在 Lead 看板完成操作后点击「确认提交」',
    steps: ['阅读发布/排期背景', '标指标、分任务或确认报告', '完成后点击底部提交'],
  },
  loginapp: {
    action: '在登录 App 完成用例后点击「确认提交」',
    steps: ['阅读冒烟用例说明', '点击获取验证码或登录观察现象', '复现/验证成功后提交'],
  },
  paymentapp: {
    action: '在支付 App 完成用例后点击「确认提交」',
    steps: ['阅读支付/回调用例说明', '点击微信支付观察状态与通知', '复现/验证成功后提交'],
  },
}
