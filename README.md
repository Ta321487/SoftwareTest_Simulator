...
备考 → 面试 → 笔试 → 入职 → 进阶 Lead。在 Jira、终端、企微、API 调试等模拟工具里完成 48 关主线（第一季 40 + 进阶 8），攒 XP 升职级。
...
| 模块            | 说明                                                                      |
| --------------- | ------------------------------------------------------------------------- |
| 主线 48 关      | 四阶段 + 进阶线：备考、面试、笔试、入职、Lead（第一季全通后解锁 8 关）    |
| 番外 50 关      | **排查工具链**（推荐 27 关）+ **思维选修**（23 关）+ 每日特训，按进度解锁 |
| 每日特训        | 通关登录模块第 5 关后解锁，按日期轮换                                     |
| 项目剧本        | 登录/支付/订单/值班/Lead 五条线的 **上机实操**（独立入口，不影响通关）    |
| 测试手札 · 百科 | 关卡复盘 + 套路卡 + 术语（含 SQL/Redis/Mock/CI 等），统一搜索             |
