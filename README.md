...
| 番外     | **89 关** | #101 起，分三章（见下），按主线进度或番外链式解锁                             |
| 每日特训 | 1 题/日   | 完成登录模块第 5 关后解锁，按日期轮换                                         |
| 项目剧本 | 5 条线    | 登录 / 支付 / 订单 / 值班 / Lead，带 Day 进度与可选上机实操                   |

> 番外三章：**测试思维选修** 23 关 · **排查工具链** 27 关（推荐） · **业务场景选修** 39 关（UAT、退款、值班、越权、压测、App 冒烟等）。
...
| 章节                  | 关数 | 侧重                                                                |
| --------------------- | ---- | ------------------------------------------------------------------- |
| 第一章 · 测试思维选修 | 23   | 安全、性能、协作、兼容、策略等场景判断                              |
| 第二章 · 排查工具链   | 27   | Linux → SQL → Redis → CI → Mock → APM → Git → MQ（**推荐路线**）    |
| 第三章 · 业务场景选修 | 39   | UAT、退款、短信、灰度、抓包、卡单、值班、Lead、越权、压测、App 冒烟 |
